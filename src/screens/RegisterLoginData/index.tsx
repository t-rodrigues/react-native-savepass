...
export function RegisterLoginData() {
  ...
  async function handleRegister(formData: FormData) {
    ...
    try {
      const loadLoginList = await AsyncStorage.getItem(dataKey);

      if(loadLoginList) {
        const loginList = JSON.parse(loadLoginList);
        const newLoginList = JSON.stringify([...loginList, newLoginData]);

        await AsyncStorage.setItem(dataKey, newLoginList);
      } else {
        await AsyncStorage.setItem(dataKey, JSON.stringify([newLoginData]));
      }
      return navigate('Home')
    } catch (error) {
      Alert.alert('Deu ruim!', 'Não foi possível salvar')
    }
  }
